...
class NeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05):
        """Trains the neural network using a fixed number of iterations.

        Parameters:
        X (numpy.ndarray): Input data matrix with shape (nx, m).
        Y (numpy.ndarray): Target ground truth vector with shape (1, m).
        iterations (int): Training loops count.
        alpha (float): Scaling factor step size.

        Returns:
        The binary matrix evaluation predictions along with the final cost.
        """
