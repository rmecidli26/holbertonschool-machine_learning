...
class Neuron:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
              graph=True, step=100):
        """Trains the neuron tracking metrics over a loop."""
        ...
        for i in range(iterations + 1):
            if i == 0:
                ...
                # Recompute activation following gradient descent adjustments
                A = self.forward_prop(X)

            if i % step == 0 or i == iterations:
                cost_val = self.cost(Y, A)
                if i not in steps_recorded:
                    steps_recorded.append(i)
                    costs_recorded.append(cost_val)
                if verbose:
