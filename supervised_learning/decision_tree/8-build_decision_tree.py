...
Provides classes for Tree Implementation using Gini and Random split.
"""
import numpy as np
...
class Leaf:
    ...
    def update_indicator(self):
        ...
        def indicator(A):
            """Indicator function for elements belonging to the leaf."""
...
class Decision_Tree:
    ...
    def possible_thresholds(self, node, feature):
        """Calculates possible thresholds for a given feature."""
        arr = self.explanatory[:, feature]
        values = np.unique(arr[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Computes best Gini impurity for a single feature without loops."""
        thresh = self.possible_thresholds(node, feature)
        if thresh.size == 0:
            return 0.0, np.inf

        F = self.explanatory[node.sub_population, feature]
        T = self.target[node.sub_population]
        C = np.unique(T)

        n = F.shape[0]

        # Reshaping for fully vectorized boolean tensor (n, t, c)
        F_exp = F[:, np.newaxis, np.newaxis]
        th_exp = thresh[np.newaxis, :, np.newaxis]
        C_exp = C[np.newaxis, np.newaxis, :]
        T_exp = T[:, np.newaxis, np.newaxis]

        # Left_F[i, j, k] matches the task's exact requirement
        Left_F = (F_exp > th_exp) & (T_exp == C_exp)
        Right_F = (F_exp <= th_exp) & (T_exp == C_exp)

        # Gini impurities calculation
        left_c = np.sum(Left_F, axis=0)
        right_c = np.sum(Right_F, axis=0)

        l_tot = np.sum(left_c, axis=1, keepdims=True)
        r_tot = np.sum(right_c, axis=1, keepdims=True)

        l_safe = np.where(l_tot == 0, 1, l_tot)
        r_safe = np.where(r_tot == 0, 1, r_tot)

        l_gini = 1 - np.sum((left_c / l_safe) ** 2, axis=1)
        r_gini = 1 - np.sum((right_c / r_safe) ** 2, axis=1)

        # Average Gini
        gini_avg = (l_tot.ravel() / n) * l_gini + (r_tot.ravel() / n) * r_gini

        best_idx = np.argmin(gini_avg)
        return thresh[best_idx], gini_avg[best_idx]

    def Gini_split_criterion(self, node):
        """Chooses the best feature and threshold using Gini impurity."""
        n_feat = self.explanatory.shape[1]
        X = np.array([
